...
from monte_carlo_step.mcmc import make_mcmc_step
...
except ImportError:  # Optional dependency; fallback keeps training runnable.
    trange = None
...
def train(cfg: ml_collections.ConfigDict):
    ...
    natoms = len(cfg.system.molecule)
    nfeatures = int(cfg.get('nfeatures', len(cfg.atoms[0]) + 1))

    seed_electrons_coords = int(cfg.get('seed_electrons_coords', 22))
    seed = int(cfg.get('seed', 42))
    init_width = float(cfg.get('init_width', 0.1))
    core_electrons = cfg.get('core_electrons', {})

    hf_basis = cfg.get('hf_basis', 'ccpvdz')
    hf_restricted = bool(cfg.get('hf_restricted', False))
    hf_states = int(cfg.get('hf_states', 0))
    hf_excitation_type = cfg.get('hf_excitation_type', 'ordered')

    mcmc_batch_per_device = int(cfg.get('mcmc_batch_per_device', 2))
    mcmc_steps = int(cfg.get('mcmc_steps', 10))
    mcmc_blocks = int(cfg.get('mcmc_blocks', 1))
    mcmc_width = float(cfg.get('mcmc_width', 0.1))

    clip_local_energy = float(cfg.get('clip_local_energy', 5.0))
    use_scan = bool(cfg.get('use_scan', False))
    complex_output = bool(cfg.get('complex_output', True))
    laplacian_method = cfg.get('laplacian_method', 'default')
    scf_fraction = float(cfg.get('scf_fraction', 1.0))
    t_init = int(cfg.get('t_init', 0))
    debug = bool(cfg.get('debug', False))

    learning_rate = float(cfg.get('learning_rate', 0.05))
    learning_rate_decay = float(cfg.get('learning_rate_decay', 10000.0))

    """jastrow indices calculation"""
    ...
    """electron coordinates initialization"""
    ...
    kan_init, kan_apply, orbitals_apply = make_kan_net(nspins=electrons,
                                                       charges=charges,
                                                       nelectrons=nelectrons,
                                                       nfeatures=nfeatures,
    ...
    monte_carlo = make_mcmc_step(batch_network=batch_network,
                                 batch_per_device=mcmc_batch_per_device,
                                 steps=mcmc_steps,
                                 atoms=atoms,
                                 blocks=mcmc_blocks)
    ...
    def learning_rate_schedule(t_: jnp.ndarray) -> jnp.ndarray:
        return learning_rate * jnp.power(
            (1.0 / (1.0 + (t_ / 1.0))), learning_rate_decay)
    ...
    if trange is not None:
        iterator = trange(t_init, cfg.iterations, desc='Training', dynamic_ncols=True)
    else:
        iterator = range(t_init, cfg.iterations)
    ...
    for t in iterator:
        ...
        if trange is not None:
            iterator.set_postfix(iter=t + 1, loss=f'{loss_value:.6f}')
        else:
            print(f'iter {t + 1}/{cfg.iterations} loss={loss_value:.6f}')
